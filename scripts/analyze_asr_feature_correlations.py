#!/usr/bin/env python3
"""Rank ASR features by association with ICAS labels and stenosis severity."""
...
def select_asr_numeric_feature_columns(df: pd.DataFrame) -> list[str]:
    """Return numeric feature columns produced by the ASR extractor."""
    cols: list[str] = []
    for col in df.columns:
        if not col.startswith("asr_"):
            continue
        if col in {"asr_file", "asr_source_file", "asr_transcript"}:
            continue
...
def _cohens_d(negative: pd.Series, positive: pd.Series) -> float:
    ...
    if pooled_std is None or np.isnan(pooled_std) or pooled_std == 0:
        return float("nan")
...
def _binary_feature_summary(values: pd.Series, labels: pd.Series) -> dict[str, float]:
    ...
    if frame["label"].nunique() < 2 or len(frame) < 3:
        return {
            ...
            "binary_mean_neg": float("nan"),
            "binary_mean_pos": float("nan"),
        }

    if frame["value"].nunique() < 2:
...
def _severity_feature_summary(values: pd.Series, severity: pd.Series) -> dict[str, float]:
    ...
    if frame["severity"].nunique() < 2 or len(frame) < 3:
        return {
    ...
    groups = [
        grp["value"].to_numpy()
        for _, grp in frame.groupby("severity")
        if len(grp) > 0
    ]
    if len(groups) >= 2:
...
def build_feature_score_table(df: pd.DataFrame) -> pd.DataFrame:
    """Create a sortable score table for ASR features."""
    matched = df.loc[df["clinical_match_status"] == "matched"].copy()
    feature_cols = select_asr_numeric_feature_columns(matched)
    rows: list[dict[str, float | str]] = []

    for feature in feature_cols:
        binary_stats = _binary_feature_summary(matched[feature], matched["has_icas"])
        severity_stats = _severity_feature_summary(matched[feature], matched["stenosis_multiclass"])
...
def write_analysis_outputs(
    ...
    """Write ranked CSV and a concise markdown summary."""
    ...
    matched = df.loc[df["clinical_match_status"] == "matched"].copy()
    binary_top = scores.sort_values("binary_corr_abs", ascending=False)
    severity_top = scores.sort_values("severity_spearman_abs", ascending=False)
    ...
    report = "\n".join(
        [
            ...
            "## Dataset Summary",
            "",
            f"- Total ASR rows: {len(df)}",
            f"- Matched clinical rows: {len(matched)}",
            f"- Binary ICAS positives: {int(matched['has_icas'].fillna(0).sum())}",
            f"- Candidate ASR numeric features: {len(select_asr_numeric_feature_columns(matched))}",
            "",
            ...
            _format_top_table(
                ...
                [
                    "feature_name",
                    "severity_spearman_rho",
                    "severity_kruskal_pvalue",
                    "combined_score",
                ],
            ),
            "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Merged ASR + clinical feature table.",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=repo_root / "reports/asr_feature_correlation_scores.csv",
        help="Ranked feature score CSV.",
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=repo_root / "reports/asr_feature_correlation_report.md",
        help="Concise markdown analysis report.",
    )
...
def main() -> None:
    ...
    print(f"Ranked ASR features: {len(scores)}")
