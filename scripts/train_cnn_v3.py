...
Usage:
    python scripts/train_cnn_v3.py
    python scripts/train_cnn_v3.py --epochs 100 --device cuda
    python scripts/train_cnn_v3.py --augment
    python scripts/train_cnn_v3.py --region-attention
    python scripts/train_cnn_v3.py --multi-task --lambda-sev 0.3
    python scripts/train_cnn_v3.py --soft-label --lambda-sev 0.5
    python scripts/train_cnn_v3.py --model mobilenet --soft-label
"""
...
class TemperatureAugmentation:
    ...
    def __call__(
        ...
        aug_face_mask = face_mask.copy() if face_mask is not None else None

        if np.random.rand() < self.p_flip:
            aug_matrix = cv2.flip(aug_matrix, 1)
            if aug_attention is not None:
                aug_attention = cv2.flip(aug_attention, 1)
            if aug_face_mask is not None:
                aug_face_mask = cv2.flip(aug_face_mask, 1)
        ...
        if self.rotation_range > 0 or self.translation_range > 0:
            ...
            if aug_face_mask is not None:
                aug_face_mask = cv2.warpAffine(aug_face_mask, M, (w, h), borderMode=cv2.BORDER_CONSTANT, borderValue=0.0)

        if self.strategy != "baseline" and np.random.rand() < self.cutout_prob:
            aug_matrix, aug_attention = self._apply_cutout(aug_matrix, aug_attention, aug_face_mask)
    ...
    def _apply_cutout(
        ...
        h, w = matrix.shape
        scale = np.random.uniform(*self.cutout_scale_range)
        cutout_h = max(1, int(round(h * scale)))
        cutout_w = max(1, int(round(w * scale)))
        effective_face_mask = face_mask if face_mask is not None else np.ones((h, w), dtype=np.float32)
        effective_attention = attention_map if attention_map is not None else np.zeros((h, w), dtype=np.float32)
        prefer_high = (
            self.strategy == "attention_guided_mixed"
            and np.random.rand() < self.mixed_high_attention_prob
        )
        candidate = build_cutout_candidate_mask(
            self.strategy,
            effective_face_mask,
            effective_attention,
        ...
        if ys.size == 0:
            ys, xs = np.where(effective_face_mask > 0.5)
        if ys.size == 0:
            ys, xs = np.where(np.ones((h, w), dtype=bool))
        pick = np.random.randint(0, ys.size)
        cy = int(ys[pick])
        cx = int(xs[pick])
        y0 = max(0, cy - cutout_h // 2)
        x0 = max(0, cx - cutout_w // 2)
        y1 = min(h, y0 + cutout_h)
        x1 = min(w, x0 + cutout_w)

        out_matrix = matrix.copy()
        out_matrix[y0:y1, x0:x1] = 0.0
        out_attention = attention_map
        if attention_map is not None:
            out_attention = attention_map.copy()
            out_attention[y0:y1, x0:x1] = 0.0
        return out_matrix, out_attention


def build_cutout_candidate_mask(
    strategy: str,
    ...
    *,
    ...
    if strategy == "face_cutout" or attention_map is None:
        return face_region

    low_attention = face_region & (attention_map > 0.0) & (attention_map <= 0.25)
    high_attention = face_region & (attention_map >= 0.5)

    if strategy == "attention_guided_mixed" and prefer_high:
        return high_attention if high_attention.any() else face_region
    if strategy in {"attention_guided_cutout", "attention_guided_mixed"}:
        return low_attention if low_attention.any() else face_region
    return face_region
...
class TemperatureDataset(Dataset):
    ...
    def __init__(
        ...
        use_mask: bool = True,
        augment: bool = False,
        augmentation_strategy: str = "baseline",
        ...
        self.augmentation_uses_attention = augmentation_strategy.startswith("attention_guided")
        self.npy_dir = npy_dir
        self.augmentation = TemperatureAugmentation(strategy=augmentation_strategy) if augment else None
    ...
    def __getitem__(self, idx: int) -> tuple[Tensor, Tensor, Tensor, str]:
        ...
        if self.use_mask:
            ...
            if mask is None:
                face_mask_small = np.ones(self.target_size, dtype=np.float32)
            else:
                mask_small = mask
                if mask_small.shape != temp_matrix.shape:
                    mask_small = cv2.resize(mask_small, (temp_matrix.shape[1], temp_matrix.shape[0]), interpolation=cv2.INTER_NEAREST)
        ...
        if self.region_attention or self.augmentation_uses_attention:
            attention_map = self._get_attention_map(sample_id, temp_matrix.shape)
        else:
            attention_map = None

        if self.augmentation is not None:
            masked_temp, attention_map = self.augmentation(masked_temp, attention_map, face_mask_small)
...
def main():
    ...
    if args.augment:
        print(f"Augmentation strategy: {args.augmentation_strategy}")
    ...
    train_dataset = TemperatureDataset(
        ...
        augmentation_strategy=args.augmentation_strategy, region_attention=args.region_attention, npy_dir=npy_dir,
    )
    ...
    checkpoint = torch.load(args.output / "best_cnn_v3.pt", weights_only=False)
    ...
    results_path = args.output / f"cnn_v3_results_{timestamp}.json"
    with results_path.open("w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, default=str)

    history_path = args.output / f"cnn_v3_history_{timestamp}.json"
