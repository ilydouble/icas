...
class GenerateExperimentsTests(unittest.TestCase):
    ...
    def test_focused_preset_includes_attention_guided_variant(self):
        experiments = generate_experiments("focused")
        guided = next(item for item in experiments if item["strategy_name"] == "attention_guided_cutout")
        self.assertTrue(guided["params"]["augment"])
        self.assertEqual(guided["params"]["augmentation-strategy"], "attention_guided_cutout")
